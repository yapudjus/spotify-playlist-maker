...
def callback():
    ...
    if token_info:
        ...
        return(f'logged in as {token_info}')
    else: return redirect("/login")  # Redirect to app's login page
# context = ('server.crt', 'server.key')#certificate and key files

# uncomment to regen token
# app.run('m.yapudjusowndomain.fr', '7654')

# comment when generating token
sp = spotipy.Spotify(auth_manager=sp_oauth)
...
if datetime.now(pytz.timezone('europe/paris')).strftime("%H") == '00':
    ...
    dailytracks = sp.user_playlist_tracks(playlist_id=daily_playlist_url)
    all_daily = []
    print('creating all_daily variable')
    for j in range(0, int(dailytracks['total']), 100):
        try:
            tmp = sp.user_playlist_tracks(playlist_id=daily_playlist_url, limit=100, offset=j)
        except:
            print('rate limitted')
            time.sleep(10)
            tmp = sp.user_playlist_tracks(playlist_id=daily_playlist_url, limit=100, offset=j)
    ...
    for i in all_daily:
        sp.user_playlist_remove_all_occurrences_of_tracks(playlist_id=daily_playlist_url, tracks=[i])
    print('Done!')
    if datetime.now(pytz.timezone('europe/paris')).weekday() == 4 :
        print('Clearing weekly playlist...')
        weeklytracks = sp.user_playlist_tracks(playlist_id=weekly_playlist_url)
        all_weekly = []
        print('creating all_weekly variable')
        for j in range(0, int(weeklytracks['total']), 100):
            try:
                tmp = sp.user_playlist_tracks(playlist_id=weekly_playlist_url, limit=100, offset=j)
            except:
                print('rate limitted')
                time.sleep(10)
                tmp = sp.user_playlist_tracks(playlist_id=weekly_playlist_url, limit=100, offset=j)
        ...
        for i in all_weekly:
            sp.user_playlist_remove_all_occurrences_of_tracks(playlist_id=weekly_playlist_url, tracks=[i])
        print('Done!')
...
if len(all_ids) != massiveplaylist['total']:
    ...
    for j in range(0, int(massiveplaylist['total']), 100):
        try:
            tmp = sp.user_playlist_tracks(playlist_id=massive_playlist_url, limit=100, offset=j)
        except:
            print('rate limitted')
            time.sleep(10)
            tmp = sp.user_playlist_tracks(playlist_id=massive_playlist_url, limit=100, offset=j)
...
for i in toadd:
    ...
    print(i)
    try:
        playlist_tracks = sp.user_playlist_tracks(playlist_id=i)
    except:
        print('rate limitted')
        time.sleep(10)
        playlist_tracks = sp.user_playlist_tracks(playlist_id=i)
    ...
    if len(tracks) > 0:
        try:
            sp.user_playlist_add_tracks(playlist_id=massive_playlist_url, tracks=tracks, position=None)
            sp.user_playlist_add_tracks(playlist_id=daily_playlist_url, tracks=tracks, position=None)
            sp.user_playlist_add_tracks(playlist_id=weekly_playlist_url, tracks=tracks, position=None)
        except:
            print('rate limitted')
            time.sleep(5)
            try:
